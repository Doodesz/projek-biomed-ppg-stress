...
double compute_kurtosis(const double *x, int N, double mean, double stdv) {
...
int find_peaks_simple(const double *x, int N, int min_dist_samples,double threshold, int *idx_out, int max_peaks=MAX_PEAKS) {
    int count = 0;
    for (int i=1;i<N-1;i++){
...
void extract_rr_intervals(const int *peak_indices, int n_peaks, int fs, double *rr_out, int *count_out) {
    *count_out = 0;
    if (n_peaks < 2) return;
    for (int i = 1; i < n_peaks; i++) {
        double rr_sec = (peak_indices[i] - peak_indices[i-1]) / (double)fs;
...
bool ssa_reconstruct_hankel_svd(const double* x, int N, int L, int ncomp, double* y_out) {
